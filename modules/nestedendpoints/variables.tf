...
variable "target_resource_id" {
  type        = string
  description = "The resource ID of the target Traffic Manager Profile."
  nullable    = false
}
...
variable "min_child_endpoints_ipv4" {
  type        = number
  default     = null
  description = "The minimum number of IPv4 endpoints that must be available in the child profile."

  validation {
    condition     = var.min_child_endpoints_ipv4 == null || var.min_child_endpoints_ipv4 >= 1
    error_message = "Minimum child IPv4 endpoints must be at least 1 when specified."
  }
}

variable "min_child_endpoints_ipv6" {
  type        = number
  default     = null
  description = "The minimum number of IPv6 endpoints that must be available in the child profile."

  validation {
    condition     = var.min_child_endpoints_ipv6 == null || var.min_child_endpoints_ipv6 >= 1
    error_message = "Minimum child IPv6 endpoints must be at least 1 when specified."
  }
}
...
variable "timeouts" {
  ...
  default     = null
  description = "Timeout configuration for the Nested Endpoint resource operations."
}
